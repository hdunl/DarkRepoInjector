/*
 * MonoLoader.cpp - Dynamic version
 *
 * Credits:
 *   - github.com/hdunl
 *   - https://github.com/warbler/SharpMonoInjector (reference)
 *
 * This version dynamically obtains its own module path so that r.e.p.o.cheat.dll is always loaded from
 * the same folder as MonoLoader.dll. Did not realize first version was getting REPO.exe directoy, not cheat dll directory
 */
...
extern "C" __declspec(dllexport) LRESULT CALLBACK HookProc(int code, WPARAM wParam, LPARAM lParam) {
    ...
    if (code >= 0 && !initialized) {
        ...
        std::string dllPath = folder + "\\r.e.p.o.cheat.dll";
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved) {
    ...
    case DLL_PROCESS_ATTACH: {
        CreateThread(NULL, 0, [](LPVOID lpParam) -> DWORD {
            ...
            std::string folder = get_module_directory(hMod);
            std::string dllPath = folder + "\\r.e.p.o.cheat.dll";
            std::string nameSpace = "r.e.p.o_cheat";
            std::string className = "Loader";
            std::string methodName = "Init";
